    ...
    return (
        <div className='shopPage'>
            ...
            <div className='shopPage__search-cont'>
                <input className='shopPage__search-input' onInput={changeInputValue} type="text" placeholder="Поиск" />
                ...
                <div className='shopPage__products'>
                    {shopItems?.filter(item => new RegExp(searchValue, 'i').test(item.name))
                        .map((item, index) => {
                        return (
                            <ProductItem
                                key={index}
