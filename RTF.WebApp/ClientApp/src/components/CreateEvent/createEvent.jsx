...
export const CreateEvent = ({eventHandler}) => {
    ...
    const [form, setForm] = useState({name: '', startDateTime: '', endDateTime: '', description: '',
        organizers: [], coins: 0});
    const authData = useSelector(showUser);
    const [startDate, setStartDate] = useState(new Date());
    const [timeForm, setTimeForm] = useState({eventStart: '', eventEnd: '',
        startDate: new Date(String(startDate)), endDate: new Date(String(startDate))})
    ...
    // useEffect(() => {
    //     timeForm.startDate = startDate;
    //     timeForm.endDate = startDate;
    //     timeForm.startDate.setHours(timeForm.eventStart.slice(0,2), timeForm.eventStart.slice(-2), 0o0);
    //     timeForm.endDate.setHours(timeForm.eventEnd.slice(0,2), timeForm.eventEnd.slice(-2), 0o0);
    //     form.startDateTime = addDay(timeForm.startDate);
    //     form.endDateTime = addDay(timeForm.endDate);
    // },[startDate, timeForm])

    useEffect(() => {
        timeForm.startDate = new Date(startDate);
        timeForm.endDate = new Date(startDate);
        timeForm.startDate.setHours(timeForm.eventStart.slice(0,2), timeForm.eventStart.slice(-2), 0o0);
        timeForm.endDate.setHours(timeForm.eventEnd.slice(0,2), timeForm.eventEnd.slice(-2), 0o0);
        // form.startDateTime = addDay(timeForm.startDate);
        // form.endDateTime = addDay(timeForm.endDate);
    }, [form, startDate, timeForm, timeForm.endDate, timeForm.eventEnd, timeForm.eventStart, timeForm.startDate])
    ...
    const changeEventType = (event) => {
        setForm({ ...form, 'eventType' : event.value});
    }
    ...
    const sendRequest = async () => {
        timeForm.startDate.setHours(timeForm.eventStart.slice(0,2), timeForm.eventStart.slice(-2), 0o0);
        timeForm.endDate.setHours(timeForm.eventEnd.slice(0,2), timeForm.eventEnd.slice(-2), 0o0);
        form.startDateTime = timeForm.startDate;
        form.endDateTime = timeForm.endDate;
        form.startDateTime = new Date(form.startDateTime.getTime() - (form.startDateTime.getTimezoneOffset() * 60000)).toISOString()
        form.endDateTime = new Date(form.endDateTime.getTime() - (form.endDateTime.getTimezoneOffset() * 60000)).toISOString()
        if (form.name !== '' && form.description !== '' && form.coins !== 0 && ids !== [] && timeForm.eventStart < timeForm.eventEnd) {
            ...
            <form className='create-event__body' onClick={(e) => e.stopPropagation()} onSubmit={sendRequest}>
                ...
                <div style={{display:'flex', gap: '78px'}}>
                    ...
                    <div className='create-event__body-input-time__wrapper'>
                        <div className='create-event__body-input-cont'>
                            <label htmlFor='event-datepicker'>Время начала:</label>
                            <div className='create-event__body-input-time__wrapper'>
                        ...
                        <div className='create-event__body-input-cont'>
                            <label htmlFor='event-datepicker'>Время конца:</label>
                            <div className='create-event__body-input-time__wrapper'>
                                <div className='create-event__body-input-time-cont'>
                                    <input
                                        type='time'
                ...
                <div className='create-event__body-input-cont-event'>
                ...
                <div className='create-event__submit-cont'>
                    <button type='submit' className='create-event__submit-button'>Создать</button>
                </div>
